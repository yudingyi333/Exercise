...
rmse_vals2 = do(5)*{
  ...
  ##We can tell that the third model is the better than others 
  
  ## compare shares and viral  
  lm_Model <- lm_3
  ...
  ## compare shares and viral  
  lm_Model2 <- lm_32
  yhat_train_test2 = predict(lm_Model2, Onlinenews_train)
  class_train_test2 = ifelse(yhat_train_test2 > 0.5, 1, 0)
  
  confusion_in2= table(y = Onlinenews_train$viral, yhat = class_train_test2)
  confusion_in2
  sum(diag(confusion_in2))/sum(confusion_in2)
  sum(Onlinenews_test$viral)/count(Onlinenews_test)
  
  yhat_test_test2 = predict(lm_Model2, Onlinenews_test)
  class_test_test2 = ifelse(yhat_test_test2 > 0.5, 1, 0)
  
  confusion_out2 = table(y = Onlinenews_test$viral, yhat = class_test_test2)
  confusion_out2
